...
class BreakerGame {
  ...
  look(){if(this.status!=='playing'||this.turn>0)return;this.nextView=this.view==='daniel'?'panel':'daniel';this.turn=.16;this.events.push('turn');}
  reboot(i){if(this.status!=='playing'||this.age<1.5||this.turn>0||this.view!=='panel'||this.active!==-1||!this.systems[i]||this.systems[i].progress>=this.systems[i].duration)return;this.active=i;this.events.push('loading');}
  tick(dt){
    ...
    else if(this.view==='panel'||this.turn>0){this.moveIn-=dt;if(this.moveIn<=0){this.stage++;this.events.push(this.stage===4?'movementFinal':'movement');this.moveIn=7+this.random()*4;if(this.stage===4)this.lastChance=14+this.random()*6;}}
...
class BreakerView {
  ...
  begin(){
    ...
    this.sound.resetNight();this.sound.ctx?.resume();this.game=new BreakerGame();this.paused=false;this.endTime=0;this.loadBeat=0;this.root.hidden=false;this.root.className='breaker-intro';
  ...
  dispose(){document.getElementById('pause').hidden=false;this.root.hidden=true;this.game=null;this.sound.resetNight();this.sound.ctx?.resume();}
  draw(){
    const g=this.game;if(!g)return;
    const turning=g.turn>0,view=turning&&g.turn<.08?g.nextView:g.view;
    ...
    const p=turning?1-g.turn/.16:0;
    this.photo.style.opacity=turning?Math.abs(2*p-1):1;
    ...
    document.getElementById('breakerTurn').classList.toggle('turn-left',view==='panel');
  ...
  tick(dt){
    ...
    if(!this.paused){g.tick(dt);for(const e of g.events.splice(0)){if(e==='complete')this.sound.chip(2);else if(e==='loading')this.loadBeat=0;else if(e!=='win')this.sound.play(e);}if(g.status!=='playing')this.endTime+=dt;}
    if(!this.paused&&g.status==='playing'&&g.active>=0){this.loadBeat-=dt;if(this.loadBeat<=0){this.sound.loadingTone(Math.floor(g.systems[g.active].progress*4));this.loadBeat=.24;}}
    this.sound.loop('breakerOffice',g.status==='playing'?.4:0);this.sound.loop('ambiance2',g.status==='playing'?.32:0);
    this.draw();
