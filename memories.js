...
class MemoryGame {
  constructor(level, random=Math.random) {
    this.level=Math.max(1,Math.min(5,level));this.story=MEMORY_STORIES[this.level-1];
    ...
    this.targets=[[[3,2],[10,5],[16,9]],[[3,8],[10,2],[16,6]],[[4,2],[12,8],[16,4]],[[3,5],[9,9],[16,2]],[[4,8],[10,2],[16,7]]][this.level-1];this.exit=[17,2];
  ...
  move(dx,dy) {
    ...
    if(this.secret&&!this.secretSeen&&this.x>=14&&this.y<=5){
      this.secretSeen=true;this.flash=1.2;this.message=['DU VAR HER FØR.','DET ER FIRE NAVN PÅ LISTEN.','IKKE SE PÅ KAMERA 00.','HAN KJENNER IGJEN DEG.','DETTE ER IKKE DET FØRSTE MINNET.'][this.level-1];this.messageFor=5;
    }
